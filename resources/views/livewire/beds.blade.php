        ...
        <table class="table table-striped dataTable no-footer dtr-column" id="DataTables_Table_0">
            <thead>
                <tr>
                    ...
                    <th class="sorting">Ward</th>
                    <th class="sorting">Price</th>
                    <th class="sorting">Available</th>
                    <th></th>
                </tr>
            </thead>
            <tbody>
                        ...
                        <td>
                            <div class="d-inline-block"><a href="javascript:;" class="dropdown hide-arrow"
                                ...
                                <ul class="dropdown-menu dropdown-menu-end m-0">
                                    <li><a href="" wire:click.prevent="selectBed({{ $bed->id }})"
                                            class="dropdown-item">Edit</a></li>
                                    <div class="dropdown-divider"></div>
                                    <li><a id="delet{{ $bed->id }}" data-value="{{ $bed->id }}"
                                            class="dropdown-item text-danger delete-record">Delete</a></li>
                                </ul>
                            </div>
                            <script>
                                document.querySelector('#delet{{ $bed->id }}').addEventListener('click', function(e) {
                            ...
                            <form id="delet#{{ $bed->id }}" action="{{ route('app.beds.destroy', $bed->id) }}"
        ...
        <div class="row">
            <div class="col-sm-12 col-md-6">
                <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
                    Showing {{ $beds->firstItem() }} to {{ $beds->lastItem() }} of
                    {{ $beds->total() }} entries</div>
            </div>
            <div class="col-sm-12 col-md-6">
