                        ...
                        <td>
                            <div class="d-inline-block"><a href="javascript:;" class="dropdown hide-arrow"
                                ...
                                <ul class="dropdown-menu dropdown-menu-end m-0">
                                    <li><a href="" wire:click.prevent="selectStore({{ $icd->id }})"
                                            class="dropdown-item">Edit</a></li>
                                    <div class="dropdown-divider"></div>
                                    <li><a id="dele{{ $icd->id }}" data-value="{{ $icd->id }}"
                                            class="dropdown-item text-danger delete-record">Delete</a></li>
                                </ul>
                            </div>
                            <script>
                                document.querySelector('#dele{{ $icd->id }}').addEventListener('click', function(e) {
                            ...
                            <form id="dele#{{ $icd->id }}"
                                action="{{ route('app.consulting-rooms.destroy', $icd->id) }}" method="POST"
        ...
        <div class="row">
            <div class="col-sm-12 col-md-6">
                <div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">
                    Showing {{ $icds->firstItem() }} to {{ $icds->lastItem() }} of
                    {{ $icds->total() }} entries</div>
            </div>
            <div class="col-sm-12 col-md-6">
...
{{-- @include('_partials._modals.modal-new-drugs-icd') --}}
